...
from pybullet_tools.utils import (RGBA, ROOMBA_URDF, TURTLEBOT_URDF,
                                  HideOutput, LockRenderer, assign_link_colors,
                                  connect, create_box, disconnect, draw_aabb,
                                  draw_point, draw_pose, dump_body,
                                  euler_from_quat, get_aabb, get_euler,
                                  get_joint_positions, get_link_pose,
                                  get_point, get_pose, get_subtree_aabb,
                                  invert, joint_from_name, link_from_name,
                                  load_model, multiply, pairwise_collision,
                                  remove_debug, set_all_static,
                                  set_camera_pose, set_euler,
                                  set_joint_position, set_point, stable_z,
                                  unit_point, unit_quat, wait_for_duration,
                                  wait_if_gui)
...
def main(floor_width=2.0):
    ...
    world_from_obstacle = get_pose(obstacle)
    ...
    for i in range(10):
        ...
        values = get_joint_positions(robot, joints)
        print('Joint values: [x={:.3f}, y={:.3f}, yaw={:.3f}]'.format(*values))

        # Returns the pose of base_link wrt the world frame
        world_from_robot = get_link_pose(robot, base_link)
        # Decomposing pose into position and orientation (quaternion)
        position, quaternion = world_from_robot
        ...
        obstacle_from_robot = multiply(invert(world_from_obstacle), world_from_robot)

        # Computes the robot's axis-aligned bounding box (AABB)
        robot_aabb = get_subtree_aabb(robot, base_link)
        ...
        collision = pairwise_collision(robot, obstacle)
