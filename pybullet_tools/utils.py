...
WSG_50_URDF = join_paths(MODEL_DIRECTORY, 'drake/wsg_50_description/urdf/wsg_50_mesh_visual.urdf')
...
def cached_fn(fn, cache=True, **global_kargs):
    ...
    def wrapped(*args, **local_kwargs):
        ...
        local_kwargs_key = frozenset({key: key_fn(value)
                                     for key, value in local_kwargs.items()}.items())
...
MouseEvent = namedtuple('MouseEvent', ['eventType', 'mousePosX',
...
def connect(use_gui=True, shadows=True, color=None, width=None, height=None, mp4=None, fps=120):
    ...
    if use_gui:
        ...
        p.configureDebugVisualizer(p.COV_ENABLE_TINY_RENDERER, False,
                                   physicsClientId=sim_id)  # TODO: does this matter?
        p.configureDebugVisualizer(p.COV_ENABLE_SHADOWS, shadows, physicsClientId=sim_id)
        p.configureDebugVisualizer(p.COV_ENABLE_MOUSE_PICKING, False,
                                   physicsClientId=sim_id)  # mouse moves meshes
...
CameraInfo = namedtuple(
    'CameraInfo',
    ['width', 'height', 'viewMatrix', 'projectionMatrix', 'cameraUp', 'cameraForward', 'horizontal',
     'vertical', 'yaw', 'pitch', 'dist', 'target'])
...
def compute_view_matrix(target_position, distance, yaw, pitch, roll=0., z_up=True):
    view_matrix = p.computeViewMatrixFromYawPitchRoll(
        target_position, distance, math.degrees(yaw),
        math.degrees(pitch),
        math.degrees(roll),
...
def get_image(
        camera_pos=None, target_pos=None, width=640, height=480, vertical_fov=60.0, near=0.02,
        far=5.0, tiny=False, segment=False, **kwargs):
    ...
    if (camera_pos is None) or (target_pos is None):
        ...
        view_matrix = p.computeViewMatrix(
            cameraEyePosition=camera_pos, cameraTargetPosition=target_pos, cameraUpVector=up_vector,
...
def set_joint_state(body, joint, position, velocity):
    p.resetJointState(body, joint, targetValue=position,
                      targetVelocity=velocity, physicsClientId=CLIENT)
...
def create_cylinder(radius, height, mass=STATIC_MASS, color=BLUE, **kwargs):
    collision_id, visual_id = create_shape(
        get_cylinder_geometry(radius, height),
        color=color, **kwargs)
    return create_body(collision_id, visual_id, mass=mass)
...
def create_capsule(radius, height, mass=STATIC_MASS, color=BLUE, **kwargs):
    collision_id, visual_id = create_shape(
        get_capsule_geometry(radius, height),
        color=color, **kwargs)
    return create_body(collision_id, visual_id, mass=mass)
...
def create_obj(path, scale=1., mass=STATIC_MASS, color=GREY, **kwargs):
    collision_id, visual_id = create_shape(
        get_mesh_geometry(path, scale=scale),
        color=color, **kwargs)
    body = create_body(collision_id, visual_id, mass=mass)
...
VisualShapeData = namedtuple('VisualShapeData',
                             ['objectUniqueId', 'linkIndex', 'visualGeometryType', 'dimensions',
                              'meshAssetFileName', 'localVisualFrame_position',
                              'localVisualFrame_orientation', 'rgbaColor', 'textureUniqueId'])
...
def get_visual_data(body, link=BASE_LINK):
    ...
    visual_data = [VisualShapeData(*tup)
...
def clone_body(body, links=None, collision=True, visual=True, client=None):
    ...
    new_body = p.createMultiBody(
        ...
        if collision else NULL_ID, baseVisualShapeIndex=clone_visual_shape(
            body, base_link, client) if visual else NULL_ID, basePosition=base_point,
        baseOrientation=base_quat, baseInertialFramePosition=base_dynamics_info.local_inertial_pos,
        baseInertialFrameOrientation=base_dynamics_info.local_inertial_orn, linkMasses=masses,
        ...
        linkInertialFramePositions=inertial_positions,
        linkInertialFrameOrientations=inertial_orientations, linkParentIndices=parent_indices,
        linkJointTypes=joint_types, linkJointAxis=joint_axes, physicsClientId=client)
...
def get_closest_points(
    ...
    if (link1 is None) and (link2 is None):
        results = p.getClosestPoints(bodyA=body1, bodyB=body2,
...
RayResult = namedtuple('RayResult',
                       ['objectUniqueId', 'linkIndex', 'hit_fraction', 'hit_position',
                        'hit_normal'])  # TODO: store Ray here
...
def get_ray_from_to(mouseX, mouseY, farPlane=10000):
    ...
    rayTo = rayFrom + rayForward - 0.5*(np.array(horizon) - np.array(vertical)
...
def waypoints_from_path(path, difference_fn=None, tolerance=1e-3):
    if difference_fn is None:
        def difference_fn(q2, q1): return np.array(q2) - np.array(q1)  # get_difference
...
def get_moving_pairs(body, moving_joints):
    ...
    moving_links = list(
        filter(
            ...
            get_moving_links(body, moving_joints)))
...
def get_self_link_pairs(body, joints, disabled_collisions=set(), only_moving=True):
    ...
    check_link_pairs = list(
        filter(
            lambda pair: not are_links_adjacent(body, *pair),
            check_link_pairs))
    check_link_pairs = list(
        filter(
            lambda
            pair: (pair not in disabled_collisions)
...
def get_collision_fn(
    ...
    get_obstacle_aabb = cached_fn(get_buffered_aabb, cache=cache,
                                  max_distance=max_distance/2., **kwargs)
    ...
    def collision_fn(q, verbose=False):
        ...
        get_moving_aabb = cached_fn(get_buffered_aabb, cache=True,
                                    max_distance=max_distance/2., **kwargs)
...
def plan_waypoints_joint_motion(
        body, joints, waypoints, start_conf=None, obstacles=[],
        attachments=[],
        self_collisions=True, disabled_collisions=set(),
        resolutions=None, custom_limits={},
        max_distance=MAX_DISTANCE, use_aabb=False, cache=True):
    if start_conf is None:
        start_conf = get_joint_positions(body, joints)
    assert len(start_conf) == len(joints)
    collision_fn = get_collision_fn(
        body, joints, obstacles, attachments, self_collisions, disabled_collisions,
        custom_limits=custom_limits, max_distance=max_distance, use_aabb=use_aabb, cache=cache)
...
def plan_lazy_prm(start_conf, end_conf, sample_fn, extend_fn, collision_fn, **kwargs):
    ...
    for i1, i2 in edges:
        color = BLUE
        if any(
            colliding_vertices.get(i, False) for i in (i1, i2)) or colliding_vertices.get(
            (i1, i2),
                False):
            color = RED
...
def get_nonholonomic_extend_fn(body, joints, resolutions=None, angular_tol=0., **kwargs):
    ...
    def extend_fn(q1, q2):
        ...
        if angle is None:
            # TODO: average?
            return [np.append(q1[:2], aq) for aq in angular_extend_fn(q1[2:], q2[2:])]
        path = []
...
def plan_nonholonomic_motion(
        body, joints, end_conf, obstacles=[],
        attachments=[],
        self_collisions=True, disabled_collisions=set(),
        weights=None, resolutions=None, reversible=True, linear_tol=EPSILON, angular_tol=0.,
        max_distance=MAX_DISTANCE, use_aabb=False, cache=True, custom_limits={},
    ...
    extend_fn = get_nonholonomic_extend_fn(
        body, joints, resolutions=resolutions, reversible=reversible, linear_tol=linear_tol,
    ...
    if algorithm is None:
        ...
        return birrt(start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn, **kwargs)
    path = solve(
        start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn, algorithm=algorithm,
...
def smooth_path(robot, joints, path, obstacles=[], attachments=[],
    ...
    collision_fn = get_collision_fn(
        robot, joints, obstacles, attachments, self_collisions, disabled_collisions,
    ...
    curve = smooth_cubic(path, collision_fn, resolutions,
                         max_velocities, max_accelerations, **kwargs)
    return curve
...
def discretize_curve(body, joints, curve, resolutions=None, **kwargs):
    if curve is None:
        return None
    from motion_planners.trajectory.discretize import (sample_discretize_curve,
...
def control_joints(
        body, joints, positions=None, velocities=None, position_gain=None, velocity_scale=None,
        max_force=None):
    if positions is None:
        positions = get_joint_positions(body, joints)
    if velocities is None:
    ...
    if velocity_scale is not None:
        for i, joint in enumerate(joints):
            control_joint(
                body, joint, position=positions[i],
...
def inverse_kinematics_helper(robot, link, target_pose, null_space=None):
    ...
    if null_space is not None:
        ...
        kinematic_conf = p.calculateInverseKinematics(
            robot, link, target_point, lowerLimits=lower, upperLimits=upper, jointRanges=ranges,
            restPoses=rest, physicsClientId=CLIENT)
...
def inverse_kinematics(
        robot, link, target_pose, max_iterations=200, max_time=INF, custom_limits={},
        **kwargs):
    start_time = time.time()
...
def create_sub_robot(robot, first_joint, target_link):
    ...
    sub_robot = clone_body(robot, links=selected_links, visual=False,
...
def multiple_sub_inverse_kinematics(
        robot, first_joint, target_link, target_pose, max_attempts=1, max_solutions=INF,
        max_time=INF, custom_limits={},
        first_close=True, **kwargs):
...
def add_text(
        text, position=unit_point(),
        color=BLACK, lifetime=None, parent=NULL_ID, parent_link=BASE_LINK):
...
def add_line(start, end, color=BLACK, width=1, lifetime=None, parent=NULL_ID, parent_link=BASE_LINK):
    ...
    return p.addUserDebugLine(
        start, end, lineColorRGB=color[: 3],
        lineWidth=width, lifeTime=get_lifetime(lifetime),
        parentObjectUniqueId=parent, parentLinkIndex=parent_link, physicsClientId=CLIENT)
